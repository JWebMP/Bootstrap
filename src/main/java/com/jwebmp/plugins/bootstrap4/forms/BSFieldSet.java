...
import static com.jwebmp.core.base.html.attributes.GlobalAttributes.Name;
import static com.jwebmp.core.base.servlets.enumarations.ComponentTypes.FieldSet;
		...
		extends BSForm<J>
{
	...
	/**
	 * Disables all inputs in the fieldset when a call is happening on the framework
	 *
	 * @return
	 */
	public J disableOnCall()
	{
		...
		else if (!"jwCntrl.jw.isLoading".equals(getAttribute(AngularAttributes.ngDisabled.getAttributeName())))
		{
			...
			{
				disabledOn += " || " + getAttribute(AngularAttributes.ngDisabled.getAttributeName());
			}
			addAttribute(AngularAttributes.ngDisabled.getAttributeName(), disabledOn.trim());
		}
